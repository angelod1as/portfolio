---
title: Automate new blog post creation in a MD blog
createdAt: 1659195596093
description: Type a simple CLI command and let the computer do the magic
tags: [tech]
---
...
- Pass the filename so the folders and files are created correctly;
...
- openInVsCode.mjs
### .mjs?
This file is not a simple javascript file, it is a javascript **modules** (hence the `m`). This makes `node` understand the syntax we are using without the need to compile each file before running it.

Inside the `mjs` is plain `javascript` code, so if your IDE complains about the file extensions, adapt this to your usage.
## Main file
Let’s build a simple function and call it at the end of the `content.mjs` file:
...
In order to test this, we will use `nodemon` — a tool that runs `node` scripts in watch mode.  I have it installed globally using `yarn global add nodemon`.

In your console, navigate to the root of your project and run `nodemon scripts/newContent.mjs`. You will see the console waiting for you to save the file so it can re-run:
...
[nodemon] starting `node scripts/newContent.mjs`
[nodemon] clean exit - waiting for changes before restart
```

## Name and Type
**File**: `getFilename.mjs`
...
export const getFilename = () => {
	  const [, , type, fileName] = process.argv // Here we omit the two first argument. We don’t need them.
}
```
...
export const getFilename = () => {
  const [, , type, fileName] = process.argv
	...
	// In my project, I need the type property to match "blog" or "projects" because of the way my folders are organized
...
We will need to tell our script what’s the folder it will save the files we will create.
...
This function returns am object with the three necessary variables we need to continue. We will import this function (and destructure it) in our main file.
...
- Yes, `getUTCDate` is different from `getUTCDay`. We all learn that one day.
...
The issue is: what if the file **already exists?** In my use-case, I prefer to throw and error instead of overwriting.
...
We are safe to continue, so let’s plot this `writeFileSync` from `fs` there and get done with this:
...
This is the most obfuscated of all of the files. It takes advantage of `exec` from node’s `child-process`. It’s hard to read and to explain. What it does is simple: it runs in your terminal whatever the `console.log` outputs.
