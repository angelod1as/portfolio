...
At the moment of this writing, I'm closing my daily podcast. With 400 episodes (and a few bonuses), this has been a crazy ride and I am going to talk about its emotional entails it in a different post.
...
In music, a daily podcast is a riff, while more complex podcasts a fully produced song.

Film, an Instagram story vs a short-lenght movie.

Writing, a note vs a third-draft short story.
...
I assume you have a smartphone. Even if you don't, the good and old flip phones from the 2000 already came equipped with audio recording capabilities. You do not need a professional directional microphone costing hundreds (or thousands) of your local currency. Use what you have at hand first.

This is a common rookie mistake (and by rookie I mean you and I as well). We all love shiny new things and they are widely available. Most of them are either crap or absolutelly amazing. Either way, unnecessary.
...
First, you need to write the actual script. Sitting down and writing is not as easy as it looks like, especially because writing demands editing — and that's a completely different monster. You will write a phrase that sounds great in your mind but, at the time of recording it might sound weird. So you'll want to edit it. After a few hours, you will notice a simple 10 minutes episode became an essay of 40 minutes and you took 3 hours to complete.

Second, you will probably read your script while recording and that's not only recognizable, it's also boring. It's weird to listen to people reading from perfectly crafted text, and even weirder when they try to add spoken interjections in the text for the actor — in this case, you — to read.
...
On the other hand, I do recommend having some kind of theme before you start recording. In my experience, for instance, I'd ask listeners to add anonymous questions to a special website and I'd answer them while recording. It was not script, but it was a heading I could follow so I'd not lose my track and could know when it was time to end the recording.
...
It can be your room, or your car. The important part is: make it quiet but not empty — the more furniture, the better, especially cloths and foams (like sofas or beds). This reduces echo a lot.

If you don't have a quiet place or you are a minimalist living in a 100 square meters room with just one mattress, you will need to improvise. A colleague from Folha de S.Paulo newspaper started recording professional podcasts inside his closet — quiet, full of cloth, zero echo.
...
If you can (and want) to get a little bit more technical, look for apps that record in `wav` and not `mp3`. The latter is a compressed format and the former is not, so, easier to _save_ later if something does not sound good.
...
There are a lot of great, professional podcasts, that started in Audacity. I started my podcast on Adobe Audition (when I had an Adobe subscription) and downgraded for Audacity without regret. It's a great — although a little clunky — piece of software.
...
So, you got some themes written down, charged your laptop, got your recording app ready and put some whisky in the kids milk so they'd be quiet. It's time to sit and record.
...
I didn't have that energy, so I moved to...
...
After that I'd go to editing, the task that took more time. At least I'd be in an editing mood, and doing it over and over again made processes faster. I was learning the editing software better and knowing how to use with more agility.
...
After the 3rd season, though, I stopped scheduling like that and just published a batch of episodes at Sunday night (as I realised this made little diference for the readers). Again, it's about energy: if you have it for scheduling, do so. If not, no judgement.
...
So, from now on, I'll presume you will edit it yourself.
...
A few softwares don't have destructive flows. They, instead of saving the new state of the file over the last one, save a list of commands to apply. So the original file is there and, every time you open it, they re-apply the commands to the original file and show you the result. This way you can always come back to the original if you do something wrong.
...
Take this concept in life and avoid destructive changes in softwares (and in life, also, if possible).

</Parenthesis>

As you probably can see, I have no single clue why I do most of that. All was recommended by colleagues to (sorry) _make it sound better_.
...
It's possible, for instance, to record a boring, repeated behaviour, and just play it in every file you have.
...
I have two Macros configured in my workflow: `Duplicate for Backup.txt` (Audacity doesn't care for whitespace in the file name, but I'd avoid if I were you):
...
As you can see from these files, they are just a bunch of commands for the software to follow. I split it up for personal reasons, but this could be on a single file without any issue (it's nice, though, to show you can make different Macros and call them from other Macros by using `Macro_{NAME OF MACRO}` command, like in the end of `Duplicate for Backup.txt`).
